...
static unsigned long sdbm(unsigned char *str, int count) {
...
static void *hash(void *arg)
{
        ...
        for (uint32_t i = td->begin; i < td->end; i++) {
                string *s = &words[i];
                results[i] = sdbm((unsigned char *)s->c_str(), s->size());
        }
...
int main(int argc, char *argv[])
{
        ...
        KICK(argc < 5, "./sdbm_words <input_file> <output_file> <data_file> <num_threads>");
